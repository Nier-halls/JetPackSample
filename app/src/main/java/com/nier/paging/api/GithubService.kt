...
fun searchRepos(
    ...
    Log.w(TAG, "[searchRepos] request >>>>> query: $query, page: $page, itemsPerPage: $itemsPerPage: $itemsPerPage")
    ...
    service.searchRepos(apiQuery, page, itemsPerPage).enqueue(
            object : Callback<RepoSearchResponse> {
                override fun onFailure(call: Call<RepoSearchResponse>?, t: Throwable) {
                    ...
                    onError(t.message ?: "unknow error")
                }

                override fun onResponse(
                        call: Call<RepoSearchResponse>?,
                        response: Response<RepoSearchResponse>) {
                    Log.d(TAG, "got a reponse $response")
                    if (response.isSuccessful) {
                        val repos = response.body()?.items ?: emptyList()
                        onSuccess(repos)
                    } else {
                        onError(response.errorBody()?.string() ?: "Unknow error")
                    }
